...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private final Handler onRefreshCompleteHandler = new Handler() {
					...
					if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
						...
						new Thread() {
							public void run() {
								...
								if(isRefreshing()) {
									onRefreshCompleteHandler.sendEmptyMessage(0);
								}
							};
						}.start();
					} else {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String lastUpdateTimeLabel = context.getString(R.string.last_update_time);

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
					refreshingLabel , lastUpdateTimeLabel);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel, pullLabel, refreshingLabel , lastUpdateTimeLabel);
