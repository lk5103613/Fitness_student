...
//	private int mSideBuffer = 2;
	...
	private boolean isViewFlowOnTouch = false;
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		...
		mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 50);  // 最大50张图片
		init();
	}
...
*/
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if(onDrawCallback != null) {
			onDrawCallback.sendEmptyMessage(0);
		}
		
	}
	
	private boolean scrollAbled = true;
	public void setScrollAbled(boolean scrollAbled) {
		this.scrollAbled = scrollAbled;
	}

	@Override
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		LogCat .d("Cellar","scrollAbled=" + scrollAbled + "   " + ev.getAction());
		if(!scrollAbled) {
			return false;
		}
		if (getChildCount() == 0)
...
//			snapToDestination();                   修改
	...
	public void snapToDestination() {
		...
		final int whichScreen = (getScrollX() + (screenWidth / 2))  //  / 2 改为 / 1 循环滚动
				/ screenWidth;
	...
	public void scroolPrevious() {
		snapToScreen(this.getIndex() - 1);
	}
	
	public void scroolNext() {
		snapToScreen(this.getIndex() + 1);
	}
	...
	public void setSelection(int position) {
		...
		LogCat.d("Cellar","setSelection 耗时:" + (System.currentTimeMillis() - startTime));
	}
...
//		requestLayout();
	...
	private Handler onDrawCallback;
	public void setOnDrawCallback(Handler onDrawCallback) {
		this.onDrawCallback = onDrawCallback;
	}
	
	public boolean getIsViewFlowOnTouch() {
		return isViewFlowOnTouch;
	}
	public boolean setIsViewFlowOnTouch(boolean isViewFlowOnTouch) {
		return isViewFlowOnTouch;
	}
