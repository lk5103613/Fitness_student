 ...
 * </ul>
 */
public class MarginViewFlowCircleIndicator extends View implements MarginViewFlowIndicator,
		AnimationListener {
	 ...
	 * @param context
	 */
	public MarginViewFlowCircleIndicator(Context context) {
		super(context);
		//initColors(0xFFFFFFFF, 0xFFFFFFFF, STYLE_FILL, STYLE_STROKE);
		initColors(0xFF77D99B, 0xFFFFFFFF, STYLE_FILL, STYLE_STROKE);
	}
	
	public void setColors(int activeColor, int inactiveColor , int radius , int circleDistance) {
		this.circleDistance = circleDistance;
		this.radius = radius;
//		initColors(activeColor, inactiveColor, STYLE_FILL, STYLE_STROKE);
		initColors(activeColor, inactiveColor, STYLE_FILL, STYLE_FILL);
	}
	
	public void setColors(int activeColor, int inactiveColor) {
		setColors(activeColor, inactiveColor , 4 , 1);
	}
	 ...
	 * @param attrs
	 */
	public MarginViewFlowCircleIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		int activeDefaultColor = 0xFF000000;
		...
		int inactiveDefaultColor = 0x44000000;
		// Get a custom inactive color if there is one
...
//		radius = a.getDimension(R.styleable.CircleFlowIndicator_radius, 4.0f);
	...
	private void initColors(int activeColor, int inactiveColor, int activeType,
		...
		// 设置中心透明圆圈周边宽度
//		mPaintActive.setStrokeWidth((float)10.0);
		mPaintInactive.setStrokeWidth((float)4.0);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			canvas.drawCircle( (leftPadding + radius
					+ (iLoop * circleSeparation) + centeringOffset) * circleDistance,
		...
		canvas.drawCircle( (leftPadding + radius + cx+centeringOffset) * circleDistance, getPaddingTop()
	 ...
	 */
	@Override
	public void setViewPager(MarginViewFlow view) {
		resetTimer();
		viewPager = view;
		flowWidth = viewPager.getWidth();
		invalidate();
	}
